...
// -- It's in range, it's a vehicle, it's unlocked and the vehicle is in the vehicle array. So repair it!
...
player switchMove "";
// -- Action successful, send servermessage to repair vehicle. Verify item is in inventory.
//["UseItemEvent", [netID player, _iName, 'RKRep', [netID _rTarg, _qty]]] call MV_Client_fnc_SendServerMessage; redacted
// -- Get item info to get repair arguments.
private ['_iInfo', '_rCoverage', '_rLvl', '_rParts'];
...
// -- Set vehicle to be owned by the server.
{
	private ['_p'];
	_p = format ['Hit%1', _x];
	if ((_rTarg getHitPointDamage _p) > _rCoverage) then
	{
		_rTarg setHitPointDamage [_p, (_rLvl)];
	};
} foreach _rParts;
// -- Remove item from player inventory
// -- todo create cli>serv commvar event to remove inventory item.
